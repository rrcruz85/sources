    ...
    def create(self, cr, uid, ids, datas, context):
        ...
        for pedido in obj.browse(cr, uid, [datas.get('request_id', False)], context):
            ...
            cr.execute("""select 
                                ...
                                when dl.is_box_qty = TRUE and dl.qty < 1.00 then dl.bunch_per_box * dl.bunch_type::int 
                                else dl.qty end) as stems,
                                ...
                                when dl.is_box_qty = TRUE and dl.qty < 1.00 then (dl.bunch_per_box * dl.bunch_type::int * dl.sale_price)::FLOAT else (dl.qty * dl.sale_price)::FLOAT end)/
                                sum(case when dl.is_box_qty = TRUE and dl.qty >= 1.00 then dl.qty * dl.bunch_per_box * dl.bunch_type::int 
                                when dl.is_box_qty = TRUE and dl.qty < 1.00 then dl.bunch_per_box * dl.bunch_type::int else dl.qty end) as unit_price,
                                sum(case when dl.is_box_qty = TRUE and dl.qty >= 1.00 then (dl.qty * dl.bunch_per_box * dl.bunch_type::int * dl.sale_price)::FLOAT 
                                ...
                                order by dl.box_id, pp2."name", v."name", pp."name"
                                ) lines """, (pedido.id,))

                                #order by dl.box_id, pp2."name", v."name", dl.lengths, pp."name"

            lines = cr.fetchall()
            summary_supplier = {}
            summary = {}
            for l in lines:
                key = l[13] + ',' + l[0] + ',' + l[1] + ',' + l[5] + ',' +l[6] + ',' + l[10] 
                hb = 0
                qb = 0
                if l[10] == 'HB':
            ...
            count_qb = 0
            for line in lines:                
                variety = line[0].replace('&','&amp;') #line['variety']
                length = line[1] #line['length']
                unit_per_hb = line[2] #line['units']
                line_hb = line[14] if line[10] == 'HB' else 0 #line['hb']
                qb_cont = line[14] if line[10] == 'QB' else 0 #line['qb']
                count_hb += line_hb
                count_qb += qb_cont
                if count_hb > 0: switch_qb = False
                if count_qb > 0: switch_hb = False
                if count_hb >= 1:
                    switch_hb = False
                if count_qb >= 1:
                    switch_qb = False
                stems = float(line[3]) #line['stems']
                bunch = float(line[4]) #line['bunch']
                description = line[5] #line['desc']
                subclient = line[6] #line['client']
                sale_price = line[7] #line['price']
                total = float(line[8]) #line['total']   
                total_hb += line_hb             
                total_qb += qb_cont
                total_stems += stems
                total_bunch += bunch
                total_invoice += total
                total_taxes += float(line[15])
                ...
                rml += """
                        <blockTable colWidths="120.0,50.0,32.5,32.5,30.0,40.0,35.0,85.0,75.0,30.0,30.0" rowHeights="10.0" style="AllBorders">
                            <tr>
                                ...
                                <td><para style="P5_COURIER_CENTER">""" + ('1.0' if switch_hb else '*' if line_hb > 0 and line_hb < 1 else str(round(line_hb,2))) + """</para></td>
                ...
                if count_qb >= 1:
                    count_qb = 0
                    switch_qb = True
                # '1' if not switch_hb else str(round(line_hb,2))
                # '1' if not switch_qb else str(round(qb_cont,2))
                # (str(1) if switch_qb else ' ' if count_qb == 1 else str(round(qb_cont,2)))
