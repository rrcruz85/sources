...
class invoice_client_wizard(osv.osv_memory):
    ...
    def generate_invoice(self, cr, uid, ids, context=None):
        ...
        for l in obj.line_ids:                     
                ...
                stems = l.qty if not l.is_box_qty else l.qty * int(l.bunch_type) * l.bunch_per_box if l.qty >= 1.00 else int(l.bunch_type) * l.bunch_per_box
                ...
                vals = {
                    ...
                    'quantity'              : l.qty if not l.is_box_qty else l.qty * int(l.bunch_type) * l.bunch_per_box if l.qty >= 1.00 else int(l.bunch_type) * l.bunch_per_box,
                    'sequence'              : 10,
                    'uom'                   : l.uom,
                    'bunch_type'            : l.bunch_type,
...
class invoice_client_line_wizard(osv.osv_memory):
    ...
    def on_change_vals(self, cr, uid, ids, qty, purchase_price, sale_price, uom, is_box_qty, bunch_per_box, bunch_type,context=None):
        ...
        if is_box_qty and qty < 1.00:
            qty_bxs = qty
            total_purchase = int(bunch_type) * bunch_per_box * purchase_price if uom and bunch_type and bunch_per_box else 0
            total_sale = int(bunch_type) * bunch_per_box * sale_price if uom and bunch_type and bunch_per_box else 0
        elif is_box_qty and qty >= 1.00:
            qty_bxs = qty 
            total_purchase = qty * int(bunch_type) * bunch_per_box * purchase_price if uom and bunch_type and bunch_per_box else 0
            total_purchase = qty * int(bunch_type) * bunch_per_box * sale_price if uom and bunch_type and bunch_per_box else 0
        else:
            qty_bxs = int(qty / (int(bunch_type) * bunch_per_box))
            total_purhcase = qty * purchase_price
