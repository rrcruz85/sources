    ...
    def generate_requests(self, cr, uid, ids, arg, context=None):
        ...
        def generar_pedido(fecha_pedido, p, cliente_id):
            ...
            for v in p.variant_ids:
                ...
                print 'request ', v.product_id.name, v.variant_id.name, lengths, v.subclient_id.name, request_qty 
                for s in self.pool.get('purchase.request.product.variant').browse(cr, uid, v_ids):
                    ...
                    if set(sale_lengths) & set(purchase_lengths) and request_qty > 0 and s.subclient_id and v.subclient_id and s.subclient_id.id == v.subclient_id.id:
                        ...
                        #if s.template_id.partner_id.name == 'OLIMPO FLOWERS' and v.subclient_id.name == 'AL MAKTAB':
                        #    print 'aqui'
                        # se comenta porque no cruza en mas las compras
                        # cuando la SO de finca supera los tallos del pedido
                        #qty = 0
                        ...
                        l_key = str(s.template_id.partner_id.id) + ',' + str(v.product_id.id) + ',' + str(v.variant_id.id) + ',' + str(v.subclient_id.id if v.subclient_id else '') + ',' + '-'.join(purchase_lengths) + ',' + s.uom
                        tmp_dict[key].append(l_key)
                        print 'purchase ', request_qty, l_key, s.template_id.partner_id.name, v.subclient_id.name, v.product_id.name, v.variant_id.name, qty, stems_qty, lengths 
                        purchase_line_number += 1
                    ...
                    for key in tmp_dict.keys():
                        ...
                        if request_ids:
                            for line in tmp_dict[key]:
                                if line == '133,2,3,13,40,HB' or line == '133,2,3,13,40,QB':
                                    print '133,2,3,13'
                                line_vals = line.split(',')
                                purchased_ids = self.pool.get('detalle.lines').search(cr, uid, [('pedido_id', '=', pedido_id),
                                                                                                ('supplier_id', '=', int(line_vals[0])),
                                                                                                ('product_id', '=', int(line_vals[1])),
                                                                                                ('variant_id', '=',int(line_vals[2])),
                                                                                                ('subclient_id', '=',int(line_vals[3]) if line_vals[3] else False),
                                                                                                ('lengths', '=',line_vals[4]),
                                                                                                ('uom', '=',line_vals[5]),
                                                                                                ])
                                if purchased_ids:
                                    print self.pool.get('detalle.lines').browse(cr, uid, purchased_ids)[0].supplier_id.name.replace(' ','-'), self.pool.get('detalle.lines').browse(cr, uid, purchased_ids)[0].qty, self.pool.get('detalle.lines').browse(cr, uid, purchased_ids)[0].variant_id.name
