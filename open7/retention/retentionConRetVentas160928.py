        ...
        for line in inv.invoice_line:
            ...
            for tax in tax_obj.compute_all(cr, uid, line.invoice_line_tax_id, (line.price_unit* (1-(line.discount or 0.0)/100.0)) * numdays, line.quantity, inv.partner_id.id, line.product_id, inv.partner_id)['taxes']:
                ...
                if tax_group['tax_group'] in ('ret_vat_b','ret_vat_srv'):
                    if str(inv.iva_percent) == '12':
                        val['percent'] = abs(int(tax_group['amount'] / 0.12 * 100))
                    elif str(inv.iva_percent) == '14':
                ...
                if tax_group['tax_group'] in ['ret_vat_b', 'ret_vat_srv']:
                    ...
                    imp = int(str(inv.iva_percent))
                    if inv.iva_compensa == '12':
                        imp = imp - 2
                        val['amount'] = -(tax['price_unit'] * line['quantity']) * imp / 100 * ret
                ...
                if inv.type in ('out_invoice','in_invoice', 'liq_purchase'):
                    ...
                    val['base_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['base'] * tax['base_sign'], context={'date': inv.date_invoice or time.strftime('%Y-%m-%d')}, round=True)
                    val['tax_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['amount'] * tax['tax_sign'], context={'date': inv.date_invoice or time.strftime('%Y-%m-%d')}, round=True)
...
#                                        multi='all'),
        ...
        'amount_discounted': fields.function(_amount_all,
                                             ...
                                             store=VAR_STORE,
                                             multi='all'),
        'amount_compensa': fields.function(_amount_all,
                                             method=True,
                                             digits_compute=dp.get_precision('Account'),
                                             string='Compnesación IVA',
                                             store=VAR_STORE,
                                             multi='all'),
        ...
        'pais': fields.many2one('res.country', 'País al que se efectúa pago'),
        ...
        'tipo_RegFis': fields.selection([('01','Régimen general'), ('02','Paraíso fiscal'), ('03','Régimen fiscal preferente o de menor imposición')],
                            'Tipo de régimen fiscal del exterior'),
        'pais_RegGen': fields.many2one('res.country', 'País pago en régimen General'),
        'pais_ParFis': fields.many2one('res.country', 'País pago Paraíso fiscal'),
        'deno_pago': fields.char('Denominación del régimen fiscal preferente', size=256),
        'iva_percent': fields.selection([('12','12%'), ('14','14%')], 'IVA %'),
        'iva_compensa': fields.selection([('12','ZONAS AFECTADAS',), ('14','SIN COMPENSACION')], 'Compensación IVA'),
        }
