        ...
        if pedido_id and supplier_id:
            ...
            for v in pedido.purchase_line_ids:
                if v.supplier_id.id == supplier_id:                    
                    if v.agrupada:
                        cr.execute("select sum(dl.bunch_per_box) from detalle_lines dl where dl.group_id =" + str(v.group_id) + " and dl.active = true and dl.agrupada = true and dl.pedido_id = " + str(pedido_id) + 
                                    " and dl.supplier_id = " + str(supplier_id) + " and dl.product_id = " + str(v.product_id.id) +
        ...
        for o in self.browse(cr, uid, ids):
            ...
            for l in o.line_ids:
                ...
                if l.detalle_id.agrupada:
                    cr.execute("select sum(dl.bunch_per_box) from detalle_lines dl where dl.group_id =" + str(l.detalle_id.group_id) + " and dl.active = true and dl.agrupada = true and dl.pedido_id = " + str(l.invoice_id.pedido_id.id) + 
...
class confirm_invoice_line_wizard(osv.osv_memory):
    ...
    def on_change_variety(self, cr, uid, ids, pedido_id, supplier_id, product_id,variety_id, length, context=None):
        res = {'value': {}}
        if pedido_id and supplier_id and product_id and variety_id and length:
            ...
            for s_id in s_ids:
                ...
                for s in s_obj.variant_ids:
                    if s.product_id.id == product_id and s.variant_id.id == variety_id:
                        for l in s.length_ids:
                            if l.length == length:
                                res['value']['sale_price'] = l.sale_price
                                break;                         
                        break
                break
            ...
            if suppliers:
                ...
                if val_ids:
                    length_ids = self.pool.get('purchase.request.product.variant.length').search(cr, uid, [('variant_id','in', val_ids),('length','=', length)])
                    if length_ids:
                        obj = self.pool.get('purchase.request.product.variant.length').browse(cr, uid,length_ids[0])
                        res['value']['purchase_price'] = obj.purchase_price                         
        return res
