        ...
        for order in pos_order_obj.browse(self.cr, self.uid, pos_ids):
            for line in order.lines:
                line_taxes = account_tax_obj.compute_all(self.cr, self.uid, line.product_id.taxes_id,
                                                         line.price_unit, line.qty, product=line.product_id,
                                                         partner=line.order_id.partner_id or False)
                for tax in line_taxes['taxes']:
                    taxes.setdefault(tax['id'], {'name': tax['name'], 'subtotal': 0.0, 'iva_comp':0.0, 'amount': 0.0})
                    taxes[tax['id']]['subtotal'] += tax['amount']
                    taxes[tax['id']]['iva_comp'] += line.iva_compensation
                    if 'IVA' in tax['name'].upper():
                        taxes[tax['id']]['amount'] += tax['amount'] - line.iva_compensation if tax['amount'] > line.iva_compensation else 0
                    else:
...
report_sxw.report_sxw('report.pos.details', 'pos.order', 'my_pos_1/reports/pos_details.rml', parser=new_pos_details, header='internal')
