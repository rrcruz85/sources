        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids):
            for pol in pos.lines:
                result = {
                    ...
                    'iva_comp': pos.amount_iva_compensation,
        ...
        for order in pos_order_obj.browse(self.cr, self.uid, pos_ids):
            for line in order.lines:
                ...
                for tax in line_taxes['taxes']:
                    ...
                    taxes[tax['id']]['iva_comp'] += order.amount_iva_compensation
                    if 'IVA' in tax['name'].upper():
                        taxes[tax['id']]['amount'] += tax['amount'] - order.amount_iva_compensation if tax['amount'] > order.amount_iva_compensation else 0
                    else:
...
class report_pos_details(osv.AbstractModel):
    ...
    _wrapped_report_class = new_pos_details
