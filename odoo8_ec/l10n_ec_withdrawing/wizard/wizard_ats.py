    ...
    def act_export_ats(self, cr, uid, ids, context):
        ...
        for inv in inv_obj.browse(cr, uid, inv_ids):
            ...
            if inv.retention_ir or inv.no_retention_ir:
                ...
                for da in data_air:
                    ...
                    etree.SubElement(detalleAir, 'baseImpAir').text = '%.2f' % abs(da['baseImpAir'])  # noqa
            ...
            if inv.retention_id and (inv.retention_ir or inv.retention_vat):
                ...
                etree.SubElement(detallecompras, 'estabRetencion1').text = flag and inv.retention_id.name[:3] or '000'  # noqa
                etree.SubElement(detallecompras, 'ptoEmiRetencion1').text = flag and inv.retention_id.name[3:6] or '000'  # noqa
                etree.SubElement(detallecompras, 'secRetencion1').text = flag and inv.retention_id.name[6:] or '%09d' % 0  # noqa
                etree.SubElement(detallecompras, 'autRetencion1').text = flag and inv.retention_id.auth_id.name or '%010d' % 0  # noqa
                etree.SubElement(detallecompras, 'fechaEmiRet1').text = flag and self.convertir_fecha(inv.retention_id.date)  # noqa
                ###################################
            if inv.type == 'in_refund' or tcomp == '05': #considerar Nota de Debito de proveedor
                etree.SubElement(detallecompras, 'docModificado').text = '01'
                if inv.type == 'in_refund':
                    etree.SubElement(detallecompras, 'estabModificado').text = inv.origin[:3]
                    etree.SubElement(detallecompras, 'ptoEmiModificado').text = inv.origin[4:7]
                    etree.SubElement(detallecompras, 'secModificado').text = inv.origin[8:]
                    ndd_ids = inv_obj.search(cr, uid, [('state','in',['open','paid']),
                                            ('partner_id','=',inv.partner_id.id),
                                            ('type','in',['in_invoice']),
                                            ('supplier_invoice_number', '=', inv.origin[8:]),
        ...
        if float(total_ventas) > 0:
            ...
            for inv in inv_obj.browse(cr, uid, inv_ids):
                ...
                serie_emision = inv.journal_id.auth_id.serie_emision
                keyp = '%s-%s' % (part_id, tipoComprobante)
                denoCliente = inv.partner_id.name
                if inv.partner_id.tipo_persona == '6':
                    tpCliente = '01'
                else:
                    tpCliente = '02'
                partner_data = {
                ...
                if not pdata.get(keyp, False):
                    ...
                    pdata.update(partner_data)
                if not elect_obj:
                    pdata[keyp]['tipoEmision'] = 'F'
                else:
                    invid = elect_obj.search(cr, uid, [('invoice_id', '=', inv.id)])
                    if invid:
                        pdata[keyp]['tipoEmision'] = 'E'
                    else:
                        pdata[keyp]['tipoEmision'] = 'F'
                if inv.partner_id.parte_relacion:
                    pdata[keyp]['parteRel'] = 'SI'
                else:
                    pdata[keyp]['parteRel'] = 'NO'
                formapago = {'pago': inv.payment_method}
                pdata[keyp]['formaPago'].update(formapago)
                ...
                pdata[keyp]['montoIce'] += inv.amount_ice
                if inv.retention_ir:
                    data_air = self.process_lines(cr, uid, inv.tax_line)
                    for dt in data_air:
            ...
            for k, v in pdata.items():
                ...
                if v['idCliente'] == '9999999999999':
                    etree.SubElement(detalleVentas, 'tpIdCliente').text = '07'
                else:
                ...
                if tpIdCliente[v['tpIdCliente']] != '07':
                    etree.SubElement(detalleVentas, 'parteRelVtas').text = v['parteRel']
                if tpIdCliente[v['tpIdCliente']] == '06':
                    etree.SubElement(detalleVentas, 'tipoCliente').text = v['tpCliente']
                    etree.SubElement(detalleVentas, 'denoCli').text = v['denoCliente']
                etree.SubElement(detalleVentas, 'tipoComprobante').text = v['tipoComprobante']  # noqa
                etree.SubElement(detalleVentas, 'tipoEmision').text = str(v['tipoEmision'])  # noqa
                ...
                if k[-2:] != '04':
                    ...
                    for j in v['formaPago'].items():
                        etree.SubElement(forma_pago, 'formaPago').text = j[1]
        ...
        etree.SubElement(ventaEst, 'codEstab').text = serie_emision
        ...
        # Documentos Anulados
        # Modificado en 2017/06/14
        # Cambiar entidad, punto de emision y numero de documento
        # Ademas el tipo de docuemnto para la retencion a ret_in_invoice
        # Tambien se aumentan las notas de credito emitidas por la empresa
        ...
        inv_ids = inv_obj.search(cr, uid, [('state', '=', 'cancel'),
                                           ...
                                           ('type', 'in', ['out_invoice', 'out_refund']),
                                           ('company_id', '=', wiz.company_id.id)])  # noqa
        self.__logger.info("Ventas Anuladas - N/C: %s" % len(inv_ids))
        for inv in inv_obj.browse(cr, uid, inv_ids):
            ...
            etree.SubElement(detalleAnulados, 'establecimiento').text = inv.auth_inv_id.serie_entidad  # noqa
            etree.SubElement(detalleAnulados, 'puntoEmision').text = inv.auth_inv_id.serie_emision  # noqa
            etree.SubElement(detalleAnulados, 'secuencialInicio').text = str(int(inv.supplier_invoice_number))  # noqa
            etree.SubElement(detalleAnulados, 'secuencialFin').text = str(int(inv.supplier_invoice_number))  # noqa
            etree.SubElement(detalleAnulados, 'autorizacion').text = inv.auth_inv_id.name  # noqa
            anulados.append(detalleAnulados)
        # Liquidaciones de compra
        # 2017-06-16
        # Las liquidactiones de compra se ingresan como facturas de proveedor
        # la autorizacion se define en el proveedor como documento
        # liquidacin de compra y asi se refleja en el ATS
        # Para buscar las liquidaciones canceladas se modifica para buscar
        # por las autorizaciones, account_authorisation tipo liquidacin de compra type_id=3
        ...
        cr.execute(
                    "SELECT id " \
                    "FROM account_authorisation " \
                    "WHERE type_id=3 ")
        liqids = cr.fetchall()
        liqids = [id for (id,) in liqids]
        liq_ids = inv_obj.search(cr, uid, [('state', '=', 'cancel'),
                                           ('period_id', '=', period_id),
                                           ('auth_inv_id', 'in', liqids),
                                           ('company_id', '=', wiz.company_id.id)])
        for inv in inv_obj.browse(cr, uid, liq_ids):
            detalleAnulados = etree.Element('detalleAnulados')
            etree.SubElement(detalleAnulados, 'tipoComprobante').text = inv.auth_inv_id.type_id.code  # noqa
            etree.SubElement(detalleAnulados, 'establecimiento').text = inv.auth_inv_id.serie_entidad  # noqa
            etree.SubElement(detalleAnulados, 'puntoEmision').text = inv.auth_inv_id.serie_emision  # noqa
            etree.SubElement(detalleAnulados, 'secuencialInicio').text = str(int(inv.supplier_invoice_number))  # noqa
            etree.SubElement(detalleAnulados, 'secuencialFin').text = str(int(inv.supplier_invoice_number))  # noqa
        ...
        for ret in retention_obj.browse(cr, uid, ret_ids):
            ...
            etree.SubElement(detalleAnulados, 'secuencialInicio').text = str(int(ret.name[8:]))  # noqa
            etree.SubElement(detalleAnulados, 'secuencialFin').text = str(int(ret.name[8:]))  # noqa
