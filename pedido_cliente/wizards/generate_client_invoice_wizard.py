...
class invoice_client_wizard(osv.osv_memory):
        ...
        if pedido_id:
            ...
            if p_ids:
                ...
                for v in obj.line_ids:
                    qty_bxs = int(v.qty) if v.is_box_qty else  int(v.qty) / (int(v.bunch_type) * v.bunch_per_box)
                    if qty_bxs < 1:
                        qty_bxs = 1
                ...
                raise osv.except_osv('Informacion',"Para el pedido seleccionado no se ha confirmado las lineas de factura para el cliente.")
        return res
		
    def go_next(self, cr, uid, ids, context=None):

        obj = self.browse(cr, uid, ids[0])
        p_ids = self.pool.get('confirm.client.invoice').search(cr, uid, [('pedido_id', '=', obj.pedido_id.id)])
        if not p_ids:
            raise osv.except_osv('Error',"Para el pedido seleccionado no se ha confirmado las lineas de factura para el cliente.")
        ...
        for o in self.browse(cr, uid, ids):
            ...
            for v in o.line_ids:
                ...
                dict_vals = {
                    'line_number': str(i),
                    'boxes': boxes,
                    'total_purchase': v.qty * v.purchase_price if not v.is_box_qty else ((v.qty * v.bunch_per_box * int(v.bunch_type)) / uom[v.uom]) * v.purchase_price,
        ...
        self.write(cr, uid, ids,{'next':True, 'client_id': o.pedido_id.partner_id.id, 'pedido_id': o.pedido_id.id})
    ...
    def generate_invoice(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr,uid,ids):
            ...
            tmp_ids.append(self.pool.get('account.invoice').create(cr, uid, inv_vals, context))
