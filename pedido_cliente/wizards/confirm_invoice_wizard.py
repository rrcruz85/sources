    ...
    def on_change_pedido_supplier(self, cr, uid, ids, pedido_id, supplier_id, line_ids, context=None):
        ...
        if pedido_id and supplier_id:
            ...
            if p_ids:
                ...
                confirmed_lines = [l.detalle_id.id if l.detalle_id and l.detalle_id.confirmada else False for l in self.pool.get('confirm.invoice.line').browse(cr, uid, l_ids)]
            for v in pedido.purchase_line_ids:
                if v.supplier_id.id == supplier_id and v.id not in confirmed_lines:
    ...
    def confirm_invoice(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr, uid, ids):
            ...
            for l in o.line_ids:
                ...
                line += 1
                if l.detalle_id:
                    self.pool.get('detalle.lines').write(cr, uid, [l.detalle_id.id], {'confirmada': True})
